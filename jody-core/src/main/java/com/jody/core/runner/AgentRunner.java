...
import com.jody.core.circuit.CircuitBreaker;
 ...
 *   - Assemble JodyDeps for dependency injection
...
public class AgentRunner {
    ...
    /** Create JodyDeps with all subsystems wired for tool execution context. */
    private JodyDeps createDeps() {
        ...
        return new JodyDeps.Builder(config, workdir)
        ...
        for (int i = 0; i < 50; i++) {
            ...
            if (aiMsg.toolExecutionRequests() != null && !aiMsg.toolExecutionRequests().isEmpty()) {
                ...
                for (dev.langchain4j.agent.tool.ToolExecutionRequest req : aiMsg.toolExecutionRequests()) {
                    JodyTool tool = ToolRegistry.getTool(req.name());
                    if (tool == null) {
    ...
    public JodyDeps getDeps() { return deps; }
