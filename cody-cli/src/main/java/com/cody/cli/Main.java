...
import com.cody.cli.render.ConsoleRenderer;
 ...
 *
 * Usage:
 *   cody run "Create a hello world program"
 *   cody run --model claude-sonnet-4-0 "Explain this code"
 *   cody chat
 *   cody config show
 */
@Command(name = "cody", mixinStandardHelpOptions = true, version = "2.0.0",
...
public class Main implements Callable<Integer> {
    ...
    static class RunCommand implements Callable<Integer> {
        ...
        public Integer call() {
            try {
                Cody.CodyBuilder builder = Cody.builder().workdir(workdir).autoApprove(autoApprove);
                ...
                CodyClient client = builder.build();
    ...
    static class ChatCommand implements Callable<Integer> {
        ...
        public Integer call() {
            ...
            CodyClient client = Cody.builder().workdir(workdir).autoApprove(false).build();
    ...
    static class ConfigCommand implements Callable<Integer> {
        ...
        public Integer call() {
            if ("show".equals(action)) {
                Path configPath = Path.of(System.getProperty("user.home"), ".cody", "config.json");
                if (java.nio.file.Files.exists(configPath)) {
                    ...
                    System.out.println("Create one to customize Cody settings.");
                }
            }
            return 0;
        }
